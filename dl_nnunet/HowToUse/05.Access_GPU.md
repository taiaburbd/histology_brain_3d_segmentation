...
2. **Start an Interactive Session**
   - Use the following command to start an interactive session:
     ...
     qlogin -q interactif
     ```
   - This command requests an interactive session on the "interactif" queue.
...
1. **Prepare a Job Script**
   - Create a job script with the necessary commands to execute your task. For example, `my_job.sh`:
     ```sh
     #!/bin/bash
     #$ -N my_job_name
     #$ -q batch
